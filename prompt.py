...
You must ensure that the page you generate is completely consistent with the page we provide!
Please first state your plan and how you will complete this task step by step and then output the html, css and js code.
"""
...
The modified page should be dict format and no extra output.
For example:{{
...
The page information is as follows: {page_info};

Specifically, focus on:
1. Aesthetic Layout and Design:
Evaluate whether the page layout is visually appealing.
Identify any areas where the layout could be modified for better aesthetics or symmetry.
Suggest improvements in the arrangement of elements to create a more balanced and pleasing design.
2. Functionality and Completeness:
Assess if the page's existing functionality is complete and appropriate.
Identify any essential features that are currently missing and need to be added.
Provide suggestions on enhancements or additional functionalities that could improve the overall effectiveness of the webpage.
3. User Experience Enhancement:
Explore opportunities to enhance user experience by adding special effects, such as animations, hover effects, or scrolling effects.
Recommend the inclusion of buttons, interactive elements, or other UI components that can make the page more engaging.
Suggest ways to utilize colors and typography effectively to elevate the user experience without disrupting the theme and cohesiveness of the design.
4. Coordination with Theme: Ensure coordination with the theme of the website. For example, technology websites should be designed with a sense of technology, while shopping websites should have a sense of freshness.
Ensure your analysis is detailed and provides actionable insights to transform the webpage into a more polished, functional, and user-friendly design.
...
get_feedback_text_prompt = """
You are a web design maestro. The images provided show the current implementation of the webpage. Please analyze them thoroughly and identify areas for improvement. 
The whole website requirement is: {text};
The page information is as follows: {page_info};
...
get_feedback_img_prompt = """
You are a web design maestro. The first image is the desired rendering, and the second image is the rendering implemented in our current code. Please compare the differences between the two and analyze how to modify the relevant code to make the second image consistent with the first one.(In the feedback, only mention how to modify the second image, without involving information about the first image. For example, your answer should be: change the header to red instead of: change the header of the first image to red, the second image to blue, and change the header of the second image to red.)
...
get_feedback_img_text_prompt = """
You are a web design maestro. The first image is the reference image we provided, and the second image is the rendering image of our current code implementation. Please combine the information we provided to analyze the advantages of the first image, the shortcomings of the second image, and how to correct them.(In the feedback, only mention how to modify the second image, without involving information about the first image. For example, your answer should be: change the header to red instead of: change the header of the first image to red, the second image to blue, and change the header of the second image to red.)
...
Please provide a modification plan and then output the modified html, css and js code.
The output format is as follows:
Modify plan:
1.
2.
...
def get_feedback_prompt(text = None,img = None,page_info = None):
    if img and text:
        prompt = get_feedback_img_text_prompt.format(text=text,page_info=page_info)
    elif img:
        prompt = get_feedback_img_prompt.format(page_info=page_info)
    elif text:
        prompt = get_feedback_text_prompt.format(text=text,page_info=page_info)
    else:
        prompt = get_feedback_raw_prompt.format(page_info=page_info)
    return prompt
